...
public abstract class MyBot {
	 ...
	 * @return
	 */
	public abstract MyBot getSlackBot();
	 ...
	 * @param event
	 */
	public void nextConversation(MyEvent event) {
	...
	public final void reply(WebSocketSession session, MyEvent event, ExtraRichMessage reply) {
		try {
			reply.setType(EventType.MESSAGE.name().toLowerCase());
			reply.setText(complexEncode(reply.getText()));
			reply.setLinkUser("1");
			reply.setParse("full");
			if (reply.getChannel() == null && event.getChannelId() != null) {
				reply.setChannel(event.getChannelId());
	...
	private MyBotWebSocketHandler handler() {
		return new MyBotWebSocketHandler(getSlackBot());
	}
